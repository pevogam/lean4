...
variable [Monad m] [Assertion Pred] [Assertion EPred] [∀ α, WP (m α) α Pred EPred]
...
/-- `Id` has a `WP` interpretation with `Prop` assertions and no exceptions. -/
instance Id.instWP {α : Type u} : WP (Id α) α Prop EPost.Nil where
...
/-- `ExceptT` lifts a `WP` interpretation by adding an exception postcondition layer. -/
instance ExceptT.instWP {Pred : Type v}
  [Assertion Pred] [Assertion EPred] [∀ α, WP (m α) α Pred EPred] :
...
instance ExceptT.instWPMonad {Pred : Type v}
  [Monad m] [Assertion Pred] [Assertion EPred] [∀ α, WP (m α) α Pred EPred] [WPMonad m Pred EPred] :
...
theorem ExceptT.wp_apply_eq {α ε Pred EPred}
  [Assertion Pred] [Assertion EPred] [∀ α, WP (m α) α Pred EPred] (x : ExceptT ε m α)
  (post : α → Pred) (epost : EPost.Cons (ε → Pred) EPred) :
    wp x post epost = wp x.run (epost.pushExcept post) epost.tail := rfl

/-- `OptionT` lifts a `WP` interpretation by adding a `PUnit` exception postcondition layer. -/
...
theorem OptionT.wp_apply_eq {α : Type u} {Pred : Type u} {EPred}
  [Assertion Pred] [Assertion EPred] [∀ α, WP (m α) α Pred EPred] (x : OptionT m α)
  (post : α → Pred) (epost : EPost.Cons Pred EPred) :
    wp x post epost = wp x.run (epost.pushOption post) epost.tail := rfl

/-- `StateT` lifts a `WP` interpretation by adding a state argument. -/
...
theorem StateT.wp_apply_eq {σ : Type u}
  [Assertion Pred] [Assertion EPred] [∀ α, WP (m α) α Pred EPred] (x : StateT σ m α)
  (post : α → σ → Pred) (epost : EPred) (s : σ) :
    wp x post epost s = wp (x.run s) (fun (a, s) => post a s) epost := rfl

/-- `ReaderT` lifts a `WP` interpretation by adding a reader argument. -/
...
theorem ReaderT.wp_apply_eq {ρ : Type u}
  [Assertion Pred] [Assertion EPred] [∀ α, WP (m α) α Pred EPred] (x : ReaderT ρ m α)
...
/-- `Option` has a `WP` interpretation with `Prop` assertions and a single `Prop` exception
postcondition. -/
instance Option.instWP {α : Type u} : WP (Option α) α Prop Prop where
...
/-- `Except ε` has a `WP` interpretation with `Prop` assertions and a single exception
postcondition. -/
instance Except.instWP {α : Type u} : WP (Except ε α) α Prop EPost⟨ε → Prop⟩ where
...
/-- `EStateM ε σ` has a `WP` interpretation combining state and exceptions. -/
instance EStateM.instWP {α : Type} : WP (EStateM ε σ α) α (σ → Prop) (ε → σ → Prop) where
...
instance EStateM.instWPMonad : WPMonad (EStateM ε σ) (σ → Prop) (ε → σ → Prop) where
  ...
  wp_trans_bind x f := fun post epost s => by
    simp only [WP.wpTrans, bind, EStateM.bind]

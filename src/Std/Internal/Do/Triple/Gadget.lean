...
universe u v
variable {m : Type u → Type v} {Pred : Type u} {EPred : Type u} [Monad m] [Assertion Pred] [Assertion EPred] [∀ α, WP (m α) α Pred EPred] [WPMonad m Pred EPred]
...
def assertGadget [Monad m] [Assertion Pred] [Assertion EPred] [∀ α, WP (m α) α Pred EPred] [WPMonad m Pred EPred] (name : Name) (as : Pred) : m PUnit := pure ⟨⟩
...
theorem Spec.assertGadget (name : Name) (as : Pred) [Frame Pred] :
  Triple (as ⊓ (as ⇨ post ⟨⟩)) (Std.Internal.Do.assertGadget (m := m) name as) post epost := by
